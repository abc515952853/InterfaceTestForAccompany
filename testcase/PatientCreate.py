...
class PatientCreate(unittest.TestCase):
    ...
    def test_PatientCreate(self,data):
        ...
        if r.status_code == 200:
            patientinfo = self.readdb.GetPatientInfoByPhone(phone)
            if patientinfo is not None:
                self.assertEqual(patientinfo['name'],name,case_describe + api)
                self.assertEqual(patientinfo['phone'],phone,case_describe + api)
                self.assertEqual(patientinfo['relationship'],relationship,case_describe + api)
                self.assertEqual(patientinfo['phone'],phone,case_describe + api)
                self.assertEqual(patientinfo['diseases'],diseases,case_describe + api)
                self.assertEqual(patientinfo['province'],province,case_describe + api)
                self.assertEqual(patientinfo['city'],city,case_describe + api)
                self.assertEqual(patientinfo['doctorid'],doctorid,case_describe + api)
                self.assertEqual(patientinfo['assistantid'],assistantid,case_describe + api)
